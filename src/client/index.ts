 ...
 *
 * Registers a collapsible plugin card in Settings → Plugins → Plugin
 * configuration, matching the product `PluginCard` visual language. Uses the
 * same `--dsw-alias-*` design tokens and the product's chevron SVG path
 * (IconChevronDownOutline14) so it is pixel-consistent with the built-in
 * cards (Bash, Agent Loop, …).
 *
 * The form (toggles, parent-dir drag-sort, root preview, debug, save) talks
 * to the host half over JSON RPC. Dynamic-plugin `host.call` is used here so
 * the same client half works for both the dynamic and the formal host,
 * bridged via the host's RPC handle.
 *
...
export function apply(ctx) {
  ...
  styles.insert(`
    ...
    .skillScanBody { border-top: 1px solid var(--dsw-alias-border-l2); margin: 0 16px; padding-bottom: 8px; padding-top: 10px; }
    ...
    .skillScanNote { font-size: 12px; line-height: 1.5; color: var(--dsw-alias-label-tertiary); background: var(--dsw-alias-bg-layer-2); border: 1px solid var(--dsw-alias-border-l2); border-radius: 8px; padding: 8px 10px; margin-bottom: 12px; }
    .skillScanNoteBold { font-weight: 600; color: var(--dsw-alias-label-primary); }
    .skillScanNoteCode { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 11.5px; background: var(--dsw-alias-bg-module-platform); border-radius: 4px; padding: 0 4px; }
  `)

  // Product-identical IconChevronDownOutline14 SVG path (14x14, fill=currentColor).
  ...
  function SkillScanCard() {
    ...
    return h('li', { className: 'skillScanCard' + (open ? ' skillScanCardOpen' : '') },
      ...
      open ? h('div', { className: 'skillScanBody' }, h(SkillScanForm)) : null,
    )
  }

  function SkillScanForm() {
    ...
    React.useEffect(function () {
      let alive = true
      host.call('skill-scan/get-config').then(function (cfg) { if (alive) setConfig(cfg) }).catch(function () {})
      host.call('skill-scan/roots').then(function (res) { if (alive) setPreview(res) }).catch(function () {})
      return function () { alive = false }
    }, [])

    function field(updated) { setConfig(Object.assign({}, config, updated)); setSaved(false) }
    function setToggle(key, value) {
      if (key === 'scanProject' && value && config.scanParents) return field({ scanProject: true, scanParents: false })
      if (key === 'scanParents' && value && config.scanProject) return field({ scanParents: true, scanProject: false })
      field({ [key]: value })
    }
    function save() {
      setBusy(true); setError('')
      host.call('skill-scan/set-config', config).then(function (cfg) { setConfig(cfg); setSaved(true); return host.call('skill-scan/roots') })
        .then(function (res) { setPreview(res) })
        .catch(function (err) { setError(String(err && err.message ? err.message : err)) })
        .then(function () { setBusy(false) })
    }
    function refreshPreview() {
      setBusy(true); setError('')
      host.call('skill-scan/roots').then(function (res) { setPreview(res) })
        .catch(function (err) { setError(String(err && err.message ? err.message : err)) })
        .then(function () { setBusy(false) })
    }
    function runDiscover() {
      setBusy(true); setError('')
      host.call('skill-scan/discover').then(function (res) { setBugs(res) })
        .catch(function (err) { setError(String(err && err.message ? err.message : err)) })
        .then(function () { setBusy(false) })
    }
    if (config === null) return h('div', { className: 'skillScanHint' }, '加载配置…')
    ...
    function updateName(index, name) {
      ...
      field({ parentDirs: next })
    }
    function removeDir(index) {
      const next = config.parentDirs.filter(function (_, i) { return i !== index })
      field({ parentDirs: next })
    }
    function addDir() { field({ parentDirs: config.parentDirs.concat([{ name: '' }]) }) }
    function onDragStart(index) { dragIndex.current = index }
    function onDragOver(e) { e.preventDefault() }
    function onDrop(target) {
      const from = dragIndex.current
      dragIndex.current = null
      if (from === null || from === target) return
      const next = config.parentDirs.slice()
      const moved = next.splice(from, 1)[0]
      next.splice(target, 0, moved)
      field({ parentDirs: next })
    }
    function parentRow(pd, index) {
      return h('div', { key: index, className: 'skillScanPdRow', draggable: true, onDragStart: function () { onDragStart(index) }, onDragOver: onDragOver, onDrop: function () { onDrop(index) } },
        h('span', { className: 'skillScanDragHandle', title: '拖动调整优先级（越靠上越优先）' }, '⋮⋮'),
        h('input', { className: 'skillScanNameInput', value: pd.name, placeholder: '如 .claude', onChange: function (e) { updateName(index, e.target.value) } }),
        h('span', { className: 'skillScanRankOrder' }, '#' + (index + 1)),
        h('button', { type: 'button', className: 'skillScanMiniBtn', title: '删除', onClick: function () { removeDir(index) } }, '✕'))
    }
    ...
    const parentRows = (config.parentDirs || []).map(function (pd, i) { return parentRow(pd, i) })
    ...
    const bugRows = bugs && bugs.skills && bugs.skills.length > 0
      ? bugs.skills.map(function (s, i) {
          return h('div', { key: i, className: 'skillScanPreviewRow' },
            h('span', { className: 'skillScanRankBadge' }, 'rank ' + s.rank),
            h('span', { className: 'skillScanSrcBadge' }, s.source),
            h('span', { className: 'skillScanCode', style: { flex: 1 } }, s.name + '  <-  ' + s.path))
        })
      : bugs && bugs.skills ? h('div', { className: 'skillScanHint' }, '未发现任何技能') : null

    return h('div', {},
      h('div', { className: 'skillScanNote' },
        h('span', { className: 'skillScanNoteBold' }, '开源：'), ' 项目仓库 ',
        h('code', { className: 'skillScanNoteCode' }, 'github.com/sidleo/skill-scan'),
        '；安装 `dsh plugin --profile web add @sidleo/skill-scan`'),
      h('div', { className: 'skillScanSection' },
        h('div', { className: 'skillScanSectionTitle' }, '扫描层级（开关，优先级从高到低）'),
        rows),
      h('div', { className: 'skillScanSection' },
        h('div', { className: 'skillScanSectionTitle' }, 'skills 的上级目录名（可增删改，拖动排序）'),
        h('div', { className: 'skillScanHint' }, '默认 .dsh、.agents。添加一行即扫 <根>/<名称>/skills。拖动 ⋮⋮ 调整优先级，越靠上越优先。# 为当前顺序。'),
        parentRows,
      ...
      h('div', { className: 'skillScanSection' },
        ...
        h('button', { type: 'button', className: 'skillScanAddBtn', onClick: refreshPreview, disabled: busy }, '刷新预览')),
      h('div', { className: 'skillScanSection' },
        h('div', { className: 'skillScanSectionTitle' }, '实际发现技能（调试）'),
        h('button', { type: 'button', className: 'skillScanAddBtn', onClick: runDiscover, disabled: busy }, '运行发现'),
        bugs && bugs.cwd ? h('div', { className: 'skillScanHint' }, 'cwd: ' + bugs.cwd) : null,
        h('div', { className: 'skillScanPreviewBox' }, bugRows)),
      h('div', { className: 'skillScanFooter' },
        h('button', { type: 'button', className: 'skillScanSave', onClick: save, disabled: busy }, busy ? '保存中…' : '保存配置'),
        saved ? h('span', { className: 'skillScanOk' }, '✓ 已保存') : null),
